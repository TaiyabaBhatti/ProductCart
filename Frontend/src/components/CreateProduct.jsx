...
const CreateProduct = () => {
  ...
  useEffect(()=>{
    ...
    const fetchProductData = async ()=>{
      try {
        const res = await axios.get(`/api/products/get-product/${productId}`)
  ...
  const createProduct = async (data) => {
    ...
    try {
      console.log(data);
      const response = await axios.post("/api/products/upload-product", data);
      reset();
    } catch (error) {
      console.log(error);
    } finally {
   ...
   const updateProduct = async (data) => {
    ...
    try {
      console.log(data);
      const response = await axios.patch(`/api/products/update-product/${productId}`, data);
  ...
  return (
    <section className="mt-18">
      ...
      <div className="flex flex-row items-center gap-x-1.5 justify-center">
        ...
        <div className={`relative mt-10  ${productId?"bg-green-900":"bg-blue-900"} rounded-md p-5 max-w-2xl mx-auto`}>
          <form
            onSubmit={handleSubmit(productId?updateProduct:createProduct)}
          ...
          >
            ...
            <input
              ...
              className={`w-full  ${productId?"bg-green-400":"bg-blue-400"} p-1.5 rounded-xs cursor-pointer`}
            />
          </form>
